...
async function getIconUrl(domain: string, $?: CheerioAPI): Promise<string> {
  ...
  // 4. 尝试 logo.dev 图标服务
  const logoDevUrl = `https://img.logo.dev/${domain}?token=pk_XMRLa6d_THicc0IUVsTg1A`;
  if (await checkUrl(logoDevUrl)) {
    return logoDevUrl;
  }
...
async function getAllIcons(domain: string, $?: CheerioAPI): Promise<string[]> {
  ...
  const alternativeIcons = [
    ...
    `https://img.logo.dev/${domain}?token=pk_XMRLa6d_THicc0IUVsTg1A`
  ];
